...
class CodeQLAnalyzer:
    ...
    def run_codeql(self, repo_path: str, language: str | None = None) -> str:
        """Ejecuta CodeQL y devuelve el análisis en formato SARIF."""
        ...
        if not any(ruta_repo.iterdir()):
            raise ValueError(f"El repositorio esta vacío: {ruta_repo}")
    ...
    def parse_sarif(self, sarif_data: str) -> dict:
        ...
        try:
            ...
            LOGGER.error(f"SARIF inválido: {error}")
    ...
    def save_analysis(self, repo_name: str, analysis_data: dict) -> Path:
        ...
        LOGGER.info("Análisis CodeQL guardado en %s", ruta_salida.relative_to(self.project_root))
        return ruta_salida
    ...
    def _diagnosticar_entorno(self):
        ...
        for lenguaje in ["python", "javascript", "java"]:
            pack_disponible = self._verificar_query_pack(lenguaje)
            if pack_disponible:
    ...
    def _crear_base_datos_codeql(self, ruta_repo: Path, lenguaje: str) -> Path:
        ...
        if resultado.returncode != 0 and lenguaje == "javascript":
            ...
            if resultado_retry.returncode == 0:
                ...
                detalle_error = resultado_retry.stderr.strip() or "CodeQL terminó con un error desconocido."
                raise RuntimeError(
        ...
        if resultado.returncode != 0:
            detalle_error = resultado.stderr.strip() or "CodeQL terminó con un error desconocido."
            raise RuntimeError(
...
def _construir_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Muestra que repositorios se procesarían sin ejecutar CodeQL.",
    )
    parser.add_argument(
        "--diagnose",
        action="store_true",
